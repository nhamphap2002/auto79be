 ...
 * @since  1.6
 */
class Auto79ModelJobs extends JModelList {
    ...
    public function getItems() {
        ...
        foreach ($items as $oneItem) {
            ...
            if (isset($oneItem->jobid)) {
                $values = explode(',', $oneItem->jobid);

                $textValue = array();
                foreach ($values as $value) {
                    if (!empty($value)) {
                        $db = JFactory::getDbo();
                        $query = "SELECT title FROM #__job79_jobs WHERE id =" . $value;
                        $db->setQuery($query);
